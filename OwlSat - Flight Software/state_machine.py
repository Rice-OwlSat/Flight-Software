...
class StateMachine:
    ...
    def progress_state(self):
        match self.state:
            ...
            case STOWED: # Cubesat just turned ON
                self.state = DEPLOYING
                
            case DEPLOYING: # Cubesat is initialized and running - Antenna must be deployed
                if self.deployment_successful:
                    self.state = DETUMBLING
                elif not self.deployment_successful:
                    self.state = DEPLOYING
                    
            case DETUMBLING: # Cubesat is detumbling
                if self.angular_velocity_stable:
                    self.state = STABLE
                elif not self.angular_velocity_stable:
                    self.state = DETUMBLING
                    
            case STABLE: # Cubessat has detumbled - Must establish comms link
            ...
            case MISSION: # Cubesat is performing mission
                if self.angular_velocity_stable and self.battery_check_successful and self.ground_station_link_successful:
                    self.state = MISSION
                elif not self.battery_check_successful:
                    self.state = RECOVERY
